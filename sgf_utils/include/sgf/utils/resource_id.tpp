...
namespace std {
    template <typename Tag>
    size_t hash<sgf_core::ResourceId<Tag>>::operator()(const sgf_core::ResourceId<Tag> & id) const noexcept {
        return ((size_t)id.getGen()) << 32 | id.getId();
    }

    template <typename Tag>
    bool equal_to<sgf_core::ResourceId<Tag>>::operator()(const sgf_core::ResourceId<Tag> & left, const sgf_core::ResourceId<Tag> & right) const noexcept {
