...
T& Manager<T>::getRef(const TId & id) const {
    if (!isExist(id)) throw sgf::ResourceNotFound("Try to get " + T::getTypeName() + " with id " + id.toString() + " from manager, but not found");
...
void Manager<T>::remove(const TId & id) {
    if (!isExist(id)) throw sgf::ResourceNotFound("Try to remove " + T::getTypeName() + " with id " + id.toString() + " in manager, but not found");
    items.erase(id);
}
